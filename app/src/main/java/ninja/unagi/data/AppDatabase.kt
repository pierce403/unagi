...
abstract class AppDatabase : RoomDatabase() {
  ...
  companion object {
    ...
    private val MIGRATION_8_9 = object : Migration(8, 9) {
      override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("DROP INDEX IF EXISTS `index_sightings_deviceKey`")
        database.execSQL(
          "CREATE INDEX IF NOT EXISTS `index_sightings_deviceKey_timestamp` " +
            "ON `sightings` (`deviceKey`, `timestamp`)"
        )
      }
    }
